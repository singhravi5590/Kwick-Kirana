...
const UploadCategoryModel = ({close, fetchCategory}) => {
    ...
    async function handleSubmit(e){
        e.preventDefault();
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/add-category`, {
            method : "POST",
            headers : {
    ...
    <section className='fixed top-0 left-0 right-0 p-4 bottom-0 bg-neutral-800 bg-opacity-60 flex justify-center items-center'>
        <div className='bg-white w-full p-4 rounded max-w-4xl'>
            ...
            <form onSubmit={handleSubmit} className='my-3 flex flex-col gap-2'>
                ...
                <button className={`${data.name && data.image ? "bg-primary-200" : "bg-slate-200 font-semibold"} py-2`}>Add Category</button>
            </form>
        </div>
    </section>
  )
